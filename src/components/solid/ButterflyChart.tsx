...
export default function ButterflyChart(props: ButterflyChartProps) {
    ...
    const layout = createMemo(() => {
        const o = opts();
        const m = mids();
        const solve = (stats: ButterflyStat[]) => solveRadius(stats.map(pairLabel), m, width() / 2, height() / 2, o.innerRadius, o.labelOffset, o.outerRadius, true);
    ...
    const labels = createMemo(() => {
        const o = opts();
        const positions = layout().stats.map((stat, i) => polarToCartesian(0, 0, pizzaLabelRadius(pairLabel(stat).percentile, o.innerRadius, layout().radius, o.labelOffset), mids()[i]));
    ...
    return (<div class="pizza-chart-host" ref={hostEl}>
      <Show when={props.stats.length >= 2} fallback={<p class="chart-no-data">Not enough data for chart</p>}>
        <svg viewBox={`${-width() / 2} ${-height() / 2} ${width()} ${height()}`} class="pizza-chart-svg butterfly-chart-svg" role="img" aria-label="Butterfly comparison: primary entity on the left, comparison entity on the right. Slice size and color show percentile; numbers show raw values.">
