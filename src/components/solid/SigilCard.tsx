...
export default function SigilCard() {
    ...
    const cardScore = createDeckScoreReader(ctx, useProfileReads(), "sigil");
    return (<Show when={vibe()} fallback={<EmptyCard />}>
      {(_row) => (<Card id="sigil" as="article" class="sigil-card-vessel" aria-label="Sigil" score={cardScore}>
          ...
          <Show when={oracle()?.body} fallback={<p class="card-text-pending">Oracle reading pending.</p>}>
