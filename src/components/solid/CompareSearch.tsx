...
export default function CompareSearch(props: CompareSearchProps) {
    const directory = createMemo(() => getDirectory(props.sport));
    const candidates = createMemo(() => directory().filter(e => e.type === props.entityType && e.id !== props.excludeId));
    return (<div class="compare-search-row">
      <Show when={!props.selected} fallback={<div class="compare-pill">
        ...
        <div class="compare-search">
          <SearchBar scope="sport" variant="compact" entities={candidates()} placeholder={`Compare with another ${props.entityType}…`} maxSuggestions={8} autoFocus onPick={props.onSelect}/>
        </div>
      </Show>
    </div>);
}
