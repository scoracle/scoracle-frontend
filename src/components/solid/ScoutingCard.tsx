...
export default function ScoutingCard() {
    ...
    const cardScore = createDeckScoreReader(ctx, useProfileReads(), "scouting");
    return (<Show when={commentary()} fallback={<EmptyCard message="No scouting report yet."/>}>
      {(c) => (<Card id="scouting" as="article" class="scouting-card" aria-label="Scouting" score={cardScore}>
