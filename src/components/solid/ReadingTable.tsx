...
import { weekOptionsFrom } from "../../lib/utils/week";
import "./ReadingTable.css";
function PaneError(props: {
    label: string;
    err: unknown;
    reset: () => void;
}) {
...
export default function ReadingTable() {
    ...
    const presence = new Map(CARD_REGISTRY.map(tab => [tab.id,
        createMemo(() => ctx.week() ? false : deckHasContent(ctx, data, tab.id).catch(() => true)),
    ]));
    const dealt = createMemo(() => ctx.week() ? [] :
        registryTabs().filter(tab => presence.get(tab.id)!()).map(tab => tab.id));
    ...
    const heldCard = createMemo<{
        entity: string;
        card: ProfileTab;
    } | null>(previous => {
        const entity = `${ctx.sport()}|${ctx.type()}|${ctx.id()}`;
        const active = ctx.activeTab();
        return dealt().includes(active) ? { entity, card: active } : previous?.entity === entity ? previous : null;
    });
    // Resolve membership once so the deck and its rail agree. All product
    // queries start together; their bounded deadlines cap this shared wait.
    ...
    const showModel = () => activeControls().includes("model") && ctx.type() === "player" &&
        fantasySupported(ctx.sport()) && stats()?.rating?.fantasy != null;
    const showRate = () => activeControls().includes("rate") && ctx.type() === "player" &&
    ...
    const sportWeeks = createMemo(data.weeks);
    const weekSelectOptions = () => weekOptionsFrom(sportWeeks());
    return (<>
      <Select options={weekSelectOptions()} value={ctx.week() ?? ""} onChange={(w) => ctx.setWeek(w || null)} ariaLabel="Week"/>
    <Show when={anyControl() && !weekMode()}>
      <>
        <Show when={showModel()}>
          <Select options={MODEL_OPTIONS} value={ctx.scoreModel()} onChange={(m) => ctx.setScoreModel(m as ScoreModel)} ariaLabel="Model"/>
        </Show>
        <Show when={showRate()}>
          <Select options={rateOptions()} value={ctx.rateMode()} onChange={(r) => ctx.setRateMode(r as RateMode)} ariaLabel="Rate"/>
        </Show>
        <Show when={showScope()}>
          <Select options={scopeOptions()} value={ctx.scope()} onChange={(s) => ctx.setScope(s as RatingScope)} ariaLabel="Scope"/>
        </Show>
        <Show when={showSeason()}>
          <Select options={seasonOptions()} value={String(ctx.season() ?? seasons()[0] ?? "")} onChange={(v) => ctx.setSeason(Number(v))} ariaLabel="Season"/>
        </Show>
    ...
    const pickUp = (e: MouseEvent) => {
        ...
        if (target.closest("a, button, input, select, textarea, summary, [role='button']"))
            return;
        if (target.classList.contains("card-band-body") &&
    ...
    createEffect(() => ({ isLifted: lifted(), tab: activeTab() }), ({ isLifted, tab }) => {
        if (!isLifted)
            return;
        ...
        let node = paneRefs.get(tab) ?? null;
        while (node && node.parentElement && node !== document.body) {
            for (const sibling of node.parentElement.children) {
                if (sibling === node || sibling === backdropEl || sibling.hasAttribute("inert"))
                    continue;
        ...
        window.history.pushState({ ...(window.history.state ?? {}), scoracleLift: true }, "", window.location.href);
        ...
        return () => {
            ...
            const target = restoreFocus && restoreFocus.isConnected ? restoreFocus : faceRefs.get(tab);
            restoreFocus = null;
    ...
    const activeIdx = () => Math.max(0, visibleTabs().findIndex((t) => t.id === activeTab()));
    // ── Turning the deck ──────────────────────────────────────────────────
    ...
    return (<section class="reading-table" aria-label="Profile content">
      ...
      <Loading on={`${ctx.sport()}|${ctx.type()}|${ctx.id()}`} fallback={<div class="reading-table-deck">
            <LoadingCard label="Reading"/>
          </div>}>
        <Show when={visibleTabs().length > 0}>
          <NavWell items={navItems()} active={activeTab()} onSelect={ctx.setActiveTab} ariaLabel="Profile section" conditionsAriaLabel="Profile view controls" conditions={<Errored fallback={null}><Loading on={`${ctx.sport()}|${ctx.type()}|${ctx.id()}`} fallback={null}>
                <Conditions />
              </Loading></Errored>}/>
    ...
    here too. */}
            ...
            return (<div style={side() ? { "--depth": String(depth()) } : undefined} ref={(el) => paneRefs.set(pane.id, el)} class={["reading-table-pane", {
                        active: isActive(),
                    ...
                    <div class="pane-card" ref={(el) => cardRefs.set(pane.id, el)}>
                      <div class="pane-face" style={isActive() ? liftStyle() : undefined} onTransitionEnd={(e) => {
                    if (e.target === e.currentTarget && e.propertyName === 'zoom')
                        finishSetDown();
                }} role={isActive() && lifted() ? "dialog" : "tabpanel"} aria-modal={isActive() && lifted() ? "true" : undefined} aria-label={isActive() && lifted()
                    ? `${tabLabel(pane)} — ${characterName(pane.id)}`
                    : undefined} aria-hidden={isActive() ? undefined : "true"} inert={!isActive()} tabindex="-1" onClick={(e) => {
                    if (isActive())
                        pickUp(e);
                }} onKeyDown={(e) => {
                    if (isActive() && !lifted() && e.key === "Enter" && e.target === e.currentTarget) {
                        ...
                        <Errored fallback={(err, reset) => <PaneError label={pane.label} err={err()} reset={reset}/>}>
                          <Loading on={`${ctx.sport()}|${ctx.type()}|${ctx.id()}`} fallback={pane.fallback ? pane.fallback() : <LoadingCard label={pane.label}/>}>
...
never replaced, its faces are. */}
                      ...
                      <button type="button" class="pane-bring" inert={isActive()} aria-hidden={isActive() ? "true" : undefined} aria-label={`Bring the ${tabLabel(pane)} card forward — ${characterName(pane.id)}`} title={`${tabLabel(pane)} — ${characterName(pane.id)}`} onClick={() => {
                    bringUp(pane.id);
                }}/>
                    </div>
                  </div>);
