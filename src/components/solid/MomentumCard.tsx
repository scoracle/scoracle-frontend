...
function buildSpark(rows: {
    ...
    const xFor = (ms: number) => rows.length <= 1 ? W / 2 : PAD_X + ((ms - startMs) / span) * plotW;
    return {
        W,
        H,
        midY: yFor(50),
...
export default function MomentumCard() {
    ...
    const trends = useProfileRead("momentum");
    ...
    const momentumSummary = useProfileRead("summary");
    ...
    const cardScore = createDeckScoreReader(ctx, useProfileReads(), "momentum");
    ...
    const SPARK_H = 96;
    const generalSpark = createMemo(() => buildSpark(ratingEvents().map((e) => ({ t: new Date(e.start_time).getTime(), v: e.rating_pct })), SPARK_W, SPARK_H));
    const sentimentSpark = createMemo(() => buildSpark(sentimentSeries().map((v) => ({ t: new Date(v.date).getTime(), v: v.sentiment_avg })), SPARK_W, SPARK_H));
    // One sparkline block: caps label + tier-colored line/dots + date range.
    const sparkBlock = (label: string, color: string, s: Spark) => (<div class="trends-spark">
      <span class="card-micro-eyebrow trends-spark-label">{label}</span>
      <svg class="trends-sparkline" viewBox={`0 0 ${s.W} ${s.H}`} width={s.W} height={s.H} aria-hidden="true">
        <line class="trends-midline" x1={0} x2={s.W} y1={s.midY} y2={s.midY}/>
        <polyline fill="none" stroke={color} stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" points={s.line}/>
        <For each={s.dots}>
    ...
    return (<Show when={stats() ?? trends() ?? momentumSummary()} fallback={<EmptyCard />}>
      {(_d) => (<Show when={!isEmpty()} fallback={<EmptyCard />}>
          <Card id="momentum" as="article" aria-label="Trends" score={cardScore}>
            ...
            <Show when={verdict()?.body} fallback={<p class="card-text-pending">Momentum reading pending.</p>}>
