...
function PizzaChart(props: PizzaChartProps) {
    ...
    const layout = createMemo(() => {
        ...
        const solveFor = (p: PizzaChartStat[]) => solveRadius(p, m, halfW, halfH, o.innerRadius, o.labelOffset, props.options?.outerRadius);
        ...
        for (let pass = 0; pass < 2; pass++) {
            const reachOf = (s: PizzaChartStat) => pizzaLabelRadius(s.percentile, o.innerRadius, seed, o.labelOffset) + outerBlockWidth(s);
            ...
            if (Math.abs(r - seed) < 0.5)
                break;
            seed = r;
        }
        return { stats, radius };
    });
    return (<div class="pizza-chart-host" ref={hostEl}>
      <Show when={props.stats.length >= 2} fallback={<p class="chart-no-data">Not enough data for chart</p>}>
        <SingleChart stats={layout().stats} width={width()} height={height()} innerRadius={opts().innerRadius} outerRadius={layout().radius} labelOffset={opts().labelOffset}/>
      </Show>
    </div>);
}
// ─── Single Entity Chart ────────────────────────────────────────────────────
function SingleChart(props: {
    ...
    viewBox={`${-props.width / 2} ${-props.height / 2} ${props.width} ${props.height}`} class="pizza-chart-svg" style={{ display: 'block', width: '100%', height: '100%' }}>
      <For each={props.stats}>
        {(stat, i) => {
            const startAngle = () => i() * angleStep() - Math.PI / 2;
            const endAngle = () => startAngle() + angleStep();
            return (<PizzaSlice stat={stat} startAngle={startAngle()} endAngle={endAngle()} innerRadius={props.innerRadius} outerRadius={props.outerRadius} labelOffset={props.labelOffset}/>);
        }}
      </For>
    </svg>);
}
