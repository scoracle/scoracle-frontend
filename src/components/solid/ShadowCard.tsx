 ...
 * async/router primitives is load-bearing: createMemo and friends throw
 * outside a Route.
 *
...
import { getEntityMeta, type ResolvedMeta } from "../../lib/data/entity-meta.server";
...
function ShadowCard(props: ShadowCardProps) {
    ...
    return (<CardVessel as="article" class="shadow-card" title={props.meta.name}>
      <header class="shadow-card-header">
        <Show when={props.meta.photoUrl || props.meta.teamLogoUrl}>
          <div class="shadow-avatar">
            <Show when={props.meta.photoUrl} fallback={<img class="shadow-crest-lone" src={props.meta.teamLogoUrl} alt=""/>}>
 ...
 */
export async function captureShadowCard(ctx: ProfileContextValue, sourceCard: HTMLElement): Promise<Blob> {
    ...
    if (!meta)
        throw new Error("no entity meta for shadow card");
    ...
    const width = parseFloat(rootStyle.getPropertyValue("--card-width-portrait")) || 440;
    const aspect = parseFloat(rootStyle.getPropertyValue("--card-aspect-portrait")) || 0.615;
    ...
    const dispose = render(() => <ShadowCard meta={meta} type={ctx.type()}/>, host);
    try {
        ...
        if (slot && sourceBody) {
            ...
            while (clone.firstChild)
                slot.appendChild(clone.firstChild);
        }
        ...
        if (!card)
            throw new Error("shadow card failed to mount");
        ...
        if (!blob)
            throw new Error("card capture produced no image");
        return blob;
    }
    finally {
        dispose();
