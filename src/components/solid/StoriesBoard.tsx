 ...
 */
import { createMemo, Show, For, Errored, Loading } from "solid-js";
...
export default function StoriesBoard(props: StoriesBoardProps) {
    ...
    const data = createMemo(() => getStories(props.sport, isArchive() ? props.scope : null, STORIES_LIMIT));
    const stories = () => data()?.stories ?? [];
    const sportName = () => SPORT_DISPLAY[props.sport] ?? props.sport.toUpperCase();
    const scopeLine = () => {
        if (props.scope === "resolved")
            return `${sportName()} · resolved storylines · by recency`;
        if (props.scope === "dormant")
            return `${sportName()} · dormant storylines · by recency`;
        return `${sportName()} · open storylines · by cast heat`;
    };
    return (<div class="stories-board">
    <Errored fallback={(error, reset) => <Board title="Stories" deck="narratives" scope={scopeLine()}>
      <BoardError detail={String(error())} onRetry={() => { revalidate(getStories.key); reset(); }}/>
    </Board>}>
    <Loading on={`${props.sport}:${props.scope}`} fallback={<BoardLoading label={`${sportName()} stories loading`}/>}>
    <Board title="Stories" titleAsHeading deck="narratives" scope={scopeLine()} metricLabel={stories().length > 0 ? (isArchive() ? "Reports" : "Heat") : null} count={stories().length > 0 ? `${stories().length} storylines` : null} ariaLabel={`${sportName()} stories`}>

    <Show when={stories().length > 0} fallback={<BoardEmpty message={isArchive() ? "Nothing in this archive yet." : "No open storylines right now."} ariaLabel="No storylines"/>}>
              <ol class="board-register">
                <For each={stories()}>
                  {(story, i) => (<li class="board-row">
...
the storyline's title carries the row. */}
                      ...
                      <span class="story-metric" style={!isArchive() && story.heat != null ? { color: tierColor(story.heat) } : undefined}>
