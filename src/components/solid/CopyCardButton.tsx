...
export default function CopyCardButton(props: CopyCardButtonProps) {
    ...
    const supportsImageClipboard = () => typeof ClipboardItem !== "undefined" && !!navigator.clipboard?.write;
    const artifactFile = (blob: Blob) => new File([blob], `${props.filename()}.png`, { type: "image/png" });
    const canShareFile = (file: File) => typeof navigator.share === "function" &&
    ...
    return (<div class="copy-card-root">
      <button type="button" aria-label={state() === "ready" ? "Image ready — tap again to copy" : "Copy card as image"} aria-live="polite" disabled={state() === "busy"} onClick={handleClick} class={["copy-card-button", {
