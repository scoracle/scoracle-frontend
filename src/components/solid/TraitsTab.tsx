/**
 * TraitsTab — Strengths & weaknesses display (Solid.js island)
 *
 * Waits for stats data published by StatsTab, extracts notable
 * strengths/weaknesses from percentile ranks, and renders them side
 * by side. Loads reactively when the tab becomes active.
 */
...
export default function TraitsTab(props: { active: () => boolean }) {
  ...
  async function fetchTraits(): Promise<{ strengths: TraitItem[]; weaknesses: TraitItem[] } | null> {
    if (!shouldLoad()) return null;
    const statsData = await waitForPageData('stats', 5000) as StatsPageData | null;
    if (!statsData?.categories?.length) return null;
    return extractTraits(statsData.categories);
  }

  const [traits] = createResource(shouldLoad, fetchTraits);
      ...
      <Show when={shouldLoad()} fallback={
        <div class="sw-loading">
          <div class="sw-skeleton-section">
            <div class="skeleton-header" /><div class="tab-skeleton-item" /><div class="tab-skeleton-item" /><div class="tab-skeleton-item" />
          </div>
          <div class="sw-skeleton-section">
            <div class="skeleton-header" /><div class="tab-skeleton-item" /><div class="tab-skeleton-item" />
          </div>
        </div>
      }>
        <Show when={!traits.loading} fallback={
          <div class="sw-loading">
            <div class="sw-skeleton-section">
              <div class="skeleton-header" /><div class="tab-skeleton-item" /><div class="tab-skeleton-item" />
            </div>
          ...
          <Show when={traits()} fallback={
            <div class="tab-empty-state">
              {traits.error ? 'Unable to load strengths and weaknesses' : 'No statistics data available'}
            </div>
          }>
            {(result) => (
