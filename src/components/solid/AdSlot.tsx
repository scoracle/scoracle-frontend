...
const PUBLISHER_ID = import.meta.env.PUBLIC_ADSENSE_PUBLISHER_ID ?? "ca-pub-9821466912189944";
declare global {
    interface Window {
        adsbygoogle?: unknown[];
    }
...
export default function AdSlot(props: AdSlotProps) {
    ...
    return (<div class={`ad-slot ${props.class ?? ""}`} style={{
      ...
      <Show when={props.slot}>
        <ins class="adsbygoogle" style="display:block;width:100%;height:100%" data-ad-client={PUBLISHER_ID} data-ad-slot={props.slot} data-ad-format={props.format ?? "auto"} data-full-width-responsive={props.responsive ? "true" : "false"}/>
      </Show>
    </div>);
}
