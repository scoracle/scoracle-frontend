 ...
 */
import { buildShareUrl, isShareTab, type ShareEntity } from "../utils/share-url";
...
export function buildShareText(input: ShareTextInput): ShareTextOutput {
  const tab = isShareTab(input.cardId) ? input.cardId : undefined;
  const url = buildShareUrl(input.entity, tab, input.extra);
