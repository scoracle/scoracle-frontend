...
function spark(series: number[], x: number, y: number, w: number, h: number, color: string): string {
  ...
  const dots = series
    .map((v, i) => `<circle cx="${xFor(i).toFixed(1)}" cy="${yFor(v).toFixed(1)}" r="3.4" fill="${color}" stroke="#F4F1EB" stroke-width="1.2"/>`)
    .join("");
  return `<line x1="${x.toFixed(1)}" y1="${midY}" x2="${(x + w).toFixed(1)}" y2="${midY}" stroke="#9C9890" stroke-width="1" stroke-dasharray="3 5" opacity="0.5"/>
...
function score(cx: number, value: number | null, label: string): string {
  ...
  <text x="${cx}" y="122" font-family="PT Serif" font-size="22" fill="#9C9890" text-anchor="middle" letter-spacing="3">${escapeXml(label.toUpperCase())}</text>`;
}
...
function block(label: string, series: number[], color: string, x: number, y: number, w: number, h: number): string {
  if (!series.length) return "";
  return `<text x="${(x + w / 2).toFixed(1)}" y="${y.toFixed(1)}" font-family="PT Serif" font-size="20" fill="#9C9890" text-anchor="middle" letter-spacing="2">${escapeXml(label.toUpperCase())}</text>
