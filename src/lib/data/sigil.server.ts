 ...
 * voice (body/omen/voiced_at) — plus a bounded history for the trend
 * sparkline. Read by the Sigil card AND the meta corner score (both read
 * `.current.heat` — the mig-226 card-contract key names).
 *
...
export interface SigilCurrent {
    /** Holistic synthesis score (1-100). Served as `heat` since the mig-226
     *  card-contract rename (was `score`). */
    heat: number;
    ...
    /** 2-4 sentence Oracle reading — the card's voice. Served as `body` since the
     *  card-contract rename (was `reading`). Null only when the entity has never
     *  been voiced (the served row always carries its current voice, fresh or
     *  carried forward). */
    body: string | null;
...
async function fetchSigilImpl(sport: string, type: string, id: string): Promise<SigilResponse | null> {
    "use server";
    if (!sport || !id)
        return null;
