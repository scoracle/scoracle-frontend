...
describe("ratingForMode", () => {
  it("default mode returns the season-total columns", () => {
    ...
    expect(v.specialist_score).toBe(64);
  ...
  it("an alternate mode returns that rating_modes block", () => {
    const r = base({
      rating_modes: {
        per_36: {
          composite_rank: 72, composite_score: 58, specialist: 1.5, specialist_rank: 70, specialist_score: 56,
          specialty: "Playmaking",
  ...
  it("falls back to default when the requested mode block is absent", () => {
    ...
    const r = base({ rating_modes: { per_90: { composite_rank: 1, composite_score: 50, specialist: 0, specialist_rank: 0, specialist_score: 50, specialty: "x", breakdown: [], scoped_ranks: null } } });
