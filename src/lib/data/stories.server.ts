...
async function fetchStoriesImpl(sport: string, status?: string | null, limit?: number): Promise<StoriesResponse | null> {
    "use server";
    if (!sport)
        return null;
...
async function fetchStoryImpl(sport: string, id: string | number): Promise<StoryPageResponse | null> {
    "use server";
    if (!sport || id == null || id === "")
        return null;
