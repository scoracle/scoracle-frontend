...
async function fetchMomentumSummaryImpl(sport: string, type: string, id: string, season?: number | null): Promise<MomentumSummaryResponse | null> {
    "use server";
    if (!sport || !type || !id)
        return null;
    return fetchJsonOrNull<MomentumSummaryResponse>(entityProductUrl(sport, type, id, "momentum/summary", season), "momentum-summary");
}
