...
async function fetchHeadlinesImpl(sport: string, type: string, id: string, year: number, week: number): Promise<HeadlinesResponse | null> {
    "use server";
    if (!sport || !id)
        return null;
    return fetchJsonOrNull<HeadlinesResponse>(headlinesUrl(sport, type, id, year, week), "headlines");
}
