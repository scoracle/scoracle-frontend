...
const base = (over: Partial<StatsRating> = {}): StatsRating => ({
  ...
  rating: 5, rating_rank: 90, rating_score: 68,
...
describe("ratingForMode", () => {
  ...
  it("an alternate mode returns that rating_modes block", () => {
    const r = base({
      rating_modes: {
        per_36: {
          rating: 1.5, rating_rank: 72, rating_score: 58,
  ...
  it("falls back to default when the requested mode block is absent", () => {
    ...
    const r = base({ rating_modes: { per_90: { rating: 0, rating_rank: 1, rating_score: 50, breakdown: [], scoped_ranks: null } } });
