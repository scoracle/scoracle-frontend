 ...
 */
export function describeArc(centerX: number, centerY: number, innerRadius: number, outerRadius: number, startAngle: number, endAngle: number, padAngle: number = 0): string {
 ...
 */
export function sliceRadius(percentile: number, innerRadius: number, outerRadius: number): number {
...
export function percentileTierVar(percentile: number): string {
    if (percentile >= 90)
        return 'var(--percentile-elite, #16a34a)';
    if (percentile >= 75)
        return 'var(--percentile-above, #2563eb)';
    if (percentile >= 50)
        return 'var(--percentile-average, #d97706)';
    if (percentile >= 25)
        return 'var(--percentile-below, #ea580c)';
 ...
 */
export function placeWideLabelsVertical<T>(items: T[], midAngles: number[], widthOf: (item: T) => number): T[] {
