...
export function tallyFit(value: number | string, percentile: number, angleStep: number, innerRadius: number, outerRadius: number): TallyFit {
    ...
    for (let f = TALLY_FONT_MAX; f >= TALLY_FONT_MIN; f -= TALLY_FONT_STEP) {
        ...
        if (radius - f * 0.5 <= innerRadius)
            break; // no radial room for the glyphs
...
export const outerBlockWidth = (s: PizzaChartStat): number => Math.max(s.label.length * LABEL_CHAR_W, valueStr(s.value).length * VALUE_CHAR_W);
/**
 ...
 */
function fits(stats: PizzaChartStat[], mids: number[], R: number, halfW: number, halfH: number, innerRadius: number, labelOffset: number, outwardLabels: boolean): boolean {
    ...
    if (R > Math.min(halfW, halfH) - BOX_PAD)
        return false;
    for (let i = 0; i < stats.length; i++) {
        ...
        if (Math.abs(x) + reach > halfW - BOX_PAD)
            return false;
        if (Math.abs(y) + LABEL_BLOCK_HALF_H > halfH - BOX_PAD)
            return false;
    }
    return true;
}
...
export function solveRadius(stats: PizzaChartStat[], mids: number[], halfW: number, halfH: number, innerRadius: number, labelOffset: number, cap: number | undefined, outwardLabels = false): number {
    let lo = 0;
    ...
    for (let n = 0; n < 24; n++) {
        const mid = (lo + hi) / 2;
        if (fits(stats, mids, mid, halfW, halfH, innerRadius, labelOffset, outwardLabels))
            lo = mid;
        else
            hi = mid;
    }
