...
function SportDirectory(props: {
    sport: string;
    display: string;
}) {
    const players = createMemo(() => getLeaderboard(props.sport, "player", "composite", null, PLAYER_LIMIT));
    ...
    return (<Show when={playerRows().length > 0 || teamRows().length > 0}>
      <CardVessel as="section" class="profile-dir-strip" aria-label={`${props.display} profiles`}>
        <header class="profile-dir-head">
          ...
          <a class="profile-dir-more" href={`/leaderboard?sport=${props.sport.toUpperCase()}`}>
            Full leaderboard
          </a>
        </header>
          ...
          <Show when={playerRows().length > 0}>
            <div class="profile-dir-col">
              ...
              <ol class="profile-dir-rows">
                <For each={playerRows()}>
                  {(row) => (<li class="profile-dir-row">
                      <a class="profile-dir-name" href={profilePath(props.sport, row.entity_type, row.id, { name: row.name })}>
                        {row.name}
                      </a>
                      <span class="profile-dir-team">{row.team_code}</span>
                      <Show when={row.rating_score != null}>
                        <span class="profile-dir-score" style={{ color: tierColorScore(row.rating_score!) }}>
...
export default function ProfileDirectory() {
    ...
    return (<Show when={!legacyTarget()} fallback={<RouteRedirect href={legacyTarget()!}/>}>
      <Title>Browse profiles - Scoracle</Title>
      <Meta name="description" content="Browse every NBA, NFL, and football player and team on Scoracle — original ratings, vibe sentiment, momentum, stats, and curated news on each profile."/>
