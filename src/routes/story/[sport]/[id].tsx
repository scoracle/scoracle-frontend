...
import { revalidate, useParams, type RoutePreloadFuncArgs } from "@solidjs/router";
...
import { getStory, type StoryCastMember, type StoryClaim, type StoryQuote, } from "../../../lib/data/stories.server";
...
function Truncated<T>(props: {
    ...
    const [expanded, setExpanded] = createSignal(() => { props.items; return false; });
    const visible = () => (expanded() ? props.items : props.items.slice(0, props.shown));
    const toggle = () => (<button type="button" class="story-show-all" onClick={() => setExpanded((v) => !v)}>
...
export default function StoryPage() {
    ...
    const story = createMemo(() => validSport() && storyId() ? getStory(sport(), storyId()!) : null);
    const missing = () => story() === null;
    ...
    const memberHref = (member: StoryCastMember): string | null => member.entity_type === "player" || member.entity_type === "team"
    ...
    return (<Errored fallback={(error, reset) => {
            const err = error();
            return (<Board title="Story" titleAsHeading deck="narratives" scope={sportName()} ariaLabel="Story">
              <BoardError detail={err instanceof Error ? err.message : String(err)} onRetry={() => { revalidate(getStory.key); reset(); }}/>
            </Board>);
        }}>
      <Loading on={`${sport()}:${storyId()}`} fallback={<main class="story-main"><BoardLoading label="Story loading"/></main>}>
    ...
    <Show when={!missing()} fallback={<Board title="Story not found" titleAsHeading deck="narratives" scope={sportName()} ariaLabel="Story not found">
              ...
              <Show when={story()} keyed>
                    {(s) => (<Board title={s.title} titleAsHeading deck="narratives" scope={scopeLine()} count={`${s.cast.length} in the cast`} ariaLabel={`${sportName()} storyline`}>
...
day the story changed shape. */}
                        ...
                        <Show when={packet()} keyed>
                          {(p) => (<section class="story-section" aria-label="Latest packet">
                              ...
                              <Show when={quotes().length > 0}>
                                <div class="story-quotes">
                                  <Truncated items={quotes()} shown={QUOTES_SHOWN} label="quotes">
                                    {(quote) => (<blockquote class="story-quote">
                                        {quote.quote}
                          ...
                          <section class="story-section" aria-label="Cast">
                            ...
                            <ul class="story-cast">
                              <Truncated items={cast()} shown={CAST_SHOWN} label="cast members" inList>
                                {(member) => (<li class={["story-member", { "story-member-departed": Boolean(member.left_at) }]}>
                                    <Show when={memberHref(member)} fallback={<span class="story-member-name">{member.name}</span>}>
                        ...
                        <Show when={articles().length > 0}>
                          <section class="story-section" aria-label="Source articles">
                            ...
                            <ul class="story-articles">
                              <For each={articles()}>
                                {(article) => (<li class="story-article">
                                    <a class="story-article-title" href={article.url} target="_blank" rel="noopener noreferrer">
                                      {article.title}
                                    </a>
