...
- **`ContentShell`** — borderless layout container (a plain `<section>`, no chrome). Stacks a `<Shell>` holding a single `<NavStrip>` strip (player tabs: Composite / Specialist / Starline / Vibes / News / Leaders; teams add Roster, gated via `showFor`) and the active Card pane. All Cards are sticky-mounted — first activation runs setup, subsequent switches are a CSS flip with zero remount, zero flicker. The three rating-engine pillars: **Composite** (default tab — the datapoint pizza, fed by `getStarline().rating.rating_breakdown`), **Specialist** (illustrated peak-skill card + scarcity copy), and **Starline** (the unified rating+vibe season sparkline, formerly Trends). **Stats + Traits were retired 2026-06-02** (the percentile-pizza + strengths/weaknesses surfaces); old `?tab=stats|trends|traits|compare` links alias forward in `deriveInitialTab` (stats/traits→composite, trends→starline). The old `?vs=<id>` compare param is now inert (compare is a deferred follow-on on Composite).
...
| Nav primitive | `<NavStrip>` | tab-strip; standalone + `inline` variants. Used for the profile tabs (6 player / 7 team destinations, registry-driven via `PROFILE_TABS`), the home-page sport row, and inline rate/scope toggles. |
